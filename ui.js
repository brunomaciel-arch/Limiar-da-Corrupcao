/* ══════════════════════════════════════════════════════
   LIMIAR DA CORRUPÇÃO — webhook.js  (v5)
   Debounce de status (5s), acumulação de deltas
   ══════════════════════════════════════════════════════ */

import { getActiveAgent, calcBmMax } from './state.js';

/* ── Helpers ── */
function bar(cur, max, len = 16) {
  if (max <= 0) return '`' + '░'.repeat(len) + '`';
  const filled = Math.round(Math.max(0, Math.min(cur, max)) / max * len);
  return '`' + '█'.repeat(filled) + '░'.repeat(len - filled) + '`';
}

function clampedBar(cur, max, len = 16) {
  if (cur < 0) return '`' + '▓'.repeat(len) + '`';
  return bar(cur, max, len);
}

function qualityLabel(d1, d2, total) {
  if (d1 === 10 && d2 === 10) return '✦ Axioma';
  if (d1 === 1  && d2 === 1 ) return '✖ Absurdo';
  if (total % 2 === 0)         return '~ Fluidez';
  return '~ Atrito';
}

function rollColor(label) {
  if (label.includes('Axioma'))  return 0xffd600;
  if (label.includes('Fluidez')) return 0x00e5ff;
  if (label.includes('Atrito'))  return 0xe040fb;
  if (label.includes('Absurdo')) return 0xff1744;
  return 0xb0bec5;
}

async function post(url, payload) {
  if (!url) return false;
  try {
    const res = await fetch(url, {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify(payload),
    });
    return res.ok;
  } catch (e) {
    console.warn('[Webhook] Falha:', e);
    return false;
  }
}

/* ══════════════════════════════════════════════════════
   sendRoll
   @param {string}  skillName
   @param {number}  d1, d2, mod, bonus, total
   @param {boolean} isFree — dado livre (sem mod de atributo)
══════════════════════════════════════════════════════ */
export async function sendRoll(skillName, d1, d2, mod, bonus, total, isFree = false) {
  const agent = getActiveAgent();
  if (!agent?.webhookRolagens) return;

  const label = qualityLabel(d1, d2, total);
  const color = rollColor(label);

  const fields = [];

  if (!isFree) {
    // Perícia: mostrar d1, d2, mod e bônus separados
    fields.push({ name: 'Dado 1', value: `**${d1}**`, inline: true });
    fields.push({ name: 'Dado 2', value: `**${d2}**`, inline: true });

    const modStr   = mod   >= 0 ? `+${mod}`   : `${mod}`;
    const bonusStr = bonus >= 0 ? `+${bonus}`  : `${bonus}`;

    if (bonus !== 0) {
      fields.push({ name: 'Mod',   value: `**${modStr}**`,   inline: true });
      fields.push({ name: 'Bônus', value: `**${bonusStr}**`, inline: true });
    } else {
      fields.push({ name: 'Mod', value: `**${modStr}**`, inline: true });
    }
  } else {
    // Dado livre: mostrar total direto
    fields.push({ name: 'Expressão', value: `\`${skillName}\``, inline: true });
    if (bonus !== 0) {
      fields.push({ name: 'Bônus fixo', value: `**${bonus > 0 ? '+'+bonus : bonus}**`, inline: true });
    }
  }

  await post(agent.webhookRolagens, {
    embeds: [{
      color,
      author: {
        name: `${agent.name || '—'}${agent.title ? '  ·  ' + agent.title : ''}`,
      },
      title: `🎲  ${skillName}`,
      fields,
      footer: { text: `Total: ${total}   ${label}` },
      timestamp: new Date().toISOString(),
    }],
  });
}

/* ══════════════════════════════════════════════════════
   STATUS com debounce de 5 segundos
   Acumula todos os deltas e envia uma única mensagem
══════════════════════════════════════════════════════ */
let _statusTimer   = null;
const _pendingDeltas = { vida: 0, bm: 0, fonte: 0 };

export function sendStatus(changedField, delta) {
  // Acumular delta
  _pendingDeltas[changedField] = (_pendingDeltas[changedField] || 0) + delta;

  // Reiniciar timer
  if (_statusTimer) clearTimeout(_statusTimer);
  _statusTimer = setTimeout(flushStatus, 5000);
}

async function flushStatus() {
  _statusTimer = null;

  const agent = getActiveAgent();
  if (!agent?.webhookStatus) {
    // Limpar deltas mesmo sem webhook
    _pendingDeltas.vida  = 0;
    _pendingDeltas.bm    = 0;
    _pendingDeltas.fonte = 0;
    return;
  }

  const bmMax      = calcBmMax(agent);
  const fonteTypes = { poder: 'Poder', corrupcao: 'Corrupção', adrenalina: 'Adrenalina' };

  // Barras
  const vidaBar  = clampedBar(agent.vidaCur, agent.vidaMax);
  const bmBar    = bar(agent.bmCur, bmMax);
  const fonteBar = bar(agent.fonteCur, 10);
  const fonteType = fonteTypes[agent.fonteType] || agent.fonteType;

  const vidaStr  = `${vidaBar}  **${agent.vidaCur} / ${agent.vidaMax}**${agent.vidaCur < 0 ? '  ⚠️ zona crítica' : ''}`;
  const bmStr    = `${bmBar}  **${agent.bmCur} / ${bmMax}**`;
  const fonteStr = `${fonteBar}  **${agent.fonteCur} / 10**  ·  ${fonteType}`;

  // Resumo das mudanças
  const changes = [];
  if (_pendingDeltas.vida  !== 0) changes.push(formatDelta('Vida',            _pendingDeltas.vida));
  if (_pendingDeltas.bm    !== 0) changes.push(formatDelta('Barreira Mágica', _pendingDeltas.bm));
  if (_pendingDeltas.fonte !== 0) changes.push(formatDelta('Fonte',           _pendingDeltas.fonte));

  const footerText = changes.length > 0
    ? `Alterações: ${changes.join('  ·  ')}`
    : 'Status atualizado';

  // Cor pelo campo com maior delta absoluto
  const maxField = Object.entries(_pendingDeltas)
    .sort((a, b) => Math.abs(b[1]) - Math.abs(a[1]))[0][0];
  const colors   = { vida: 0x00e676, bm: 0x00e5ff, fonte: 0xe040fb };
  const color    = colors[maxField] ?? 0xb0bec5;

  // Limpar deltas
  _pendingDeltas.vida  = 0;
  _pendingDeltas.bm    = 0;
  _pendingDeltas.fonte = 0;

  await post(agent.webhookStatus, {
    embeds: [{
      color,
      author: {
        name: `${agent.name || '—'}${agent.title ? '  ·  ' + agent.title : ''}`,
      },
      title: '📊  Status Vitais',
      fields: [
        { name: '❤️  Vida',            value: vidaStr,  inline: false },
        { name: '🔵  Barreira Mágica', value: bmStr,    inline: false },
        { name: '🟣  Fonte',           value: fonteStr, inline: false },
      ],
      footer:    { text: footerText },
      timestamp: new Date().toISOString(),
    }],
  });
}

function formatDelta(label, delta) {
  const sign = delta > 0 ? '▲' : '▼';
  return `${label} ${sign} ${Math.abs(delta)}`;
}

/* ══════════════════════════════════════════════════════
   sendTest
══════════════════════════════════════════════════════ */
export async function sendTest(url, type) {
  const labels = {
    rolagens: '🎲  Teste de Webhook — Rolagens',
    status:   '📊  Teste de Webhook — Status',
  };
  return post(url, {
    embeds: [{
      color:       0x00e5ff,
      title:       labels[type] ?? '🔧 Teste',
      description: 'Webhook configurado com sucesso! As mensagens do Limiar da Corrupção aparecerão aqui.',
      footer:      { text: 'Limiar da Corrupção' },
      timestamp:   new Date().toISOString(),
    }],
  });
}
